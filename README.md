# Hey, I'm Mario 👋

AI Product Manager & Applied AI Consultant from Switzerland.
15+ years building digital products, most recently in banking (Raiffeisen, StarFinanz - Sparkasse).
Today I build my own apps and share the whole journey: the wins, the fails, and everything in between.


- 🚀 Building apps in public: AI-assisted, product-first, shipped fast
- 📝 Documenting the journey at [The Builders Lab](https://thebuilderslab.co) with real numbers, real decisions, including the weeks where nothing works
- 🧠 Exploring LLM-based product use cases, agents and AI-assisted development
- 🏦 Background in Banking & Fintech, so I know how regulated environments work
- 🤝 Open to collaborate, whether as Product Manager on your team or on AI & app projects
- 💬 Ask me about AI product strategy, prompt engineering, n8n, or building apps without a classic dev background
- 📫 Reach me via [message](https://tally.so/r/zxOj00) or [mario.pm](https://mario.pm)
- ⚡ Fun fact: I went from electrician to AI Product Consultant. Turns out wiring up complex technical systems is my thing either way.


## 📂 What you'll find here

- **Free projects**: selected apps, tools and experiments, free to download and use
- **Workflows, skills & templates**: ready-to-import automations and reusable building blocks to make daily (work) life a bit easier


## 📬 The Builders Lab

I write and speak about building products in public: what I'm building, what worked, what didn't, and the thinking behind it.
The wins, the fails, real numbers and the honest process. Follow along.

👉 **[thebuilderslab.co](https://thebuilderslab.co)**

--

[![Website](https://img.shields.io/badge/Web-mario.pm-000000?style=flat&logo=safari)](https://mario.pm)
[![The Builders Lab](https://img.shields.io/badge/Newsletter-The_Builders_Lab-EA4B71?style=flat&logo=substack&logoColor=white)](https://thebuilderslab.co)
